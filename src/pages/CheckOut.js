...
function CheckOut() {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = "Salta Birjar - checkout";
  ...
  let schema = Yup.object({
    fullName: Yup.string()
      .min(5, "Must be 15 characters or than")
      .max(15, "Must be 15 characters or less")
      .required("Required"),
    phone: Yup.string()
      .min(11, "Mobile number not less than 11")
      .max(14, "Mobile number not more than 11")
      .required("Required"),
    address: Yup.string()
            ...
            <Card className="shadow-sm p-4 rounded-5">
              ...
              {cartItem.length === 0 ? (
                <Alert className="mb-3">
                  You do not have a product in the goods{" "}
                  <Link to="menu">Product Order</Link>
                </Alert>
              ) : (
                <>
