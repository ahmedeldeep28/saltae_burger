...
function Cart() {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = "Salta Birjar - cart";
      ...
      <tr key={idx}>
        ...
        <th>
          <Button
            variant="danger"
            size="sm"
            onClick={() => deleteItem(item.foodName)}
          >
            delete
          </Button>
        </th>
  ...
  return (
    <>
      ...
      <Container className="min-vh-75">
        ...
        <Table
          className="rounded-4 text-center overflow-hidden shadow-sm"
          responsive
          borderless
          bordered
          hover
        ...
        <Card className="checkout_card mt-3 rounded-5 bg-light shadow-sm p-3 ms-auto">
          ...
          <ul className="p-0 unlist">
            <li className="d-flex justify-content-between pb-2 border-bottom mt-2">
              Subtotal <span>${totalCart.toFixed(2)}</span>
            </li>
            {cartItem.length === 0 ? null : (
              <>
                <li className="d-flex justify-content-between pb-2 border-bottom mt-3">
                  Shipping <span>5/pound</span>
                </li>
                <li className="d-flex justify-content-between pb-2 border-bottom mt-3">
                  order total <span>{(totalCart + 5).toFixed(2)}/pound</span>
                </li>
              </>
            )}
            <li className="d-flex  mt-3 text-success">
              Paiement when recieving
            </li>
          </ul>
          {cartItem.length === 0 ? (
            <Button className="text-capitalize mt-3" as={Link} to="/menu">
              product order
            </Button>
          ) : (
            <Button className="text-capitalize mt-3" as={Link} to="/checkout">
              checkout order
            </Button>
          )}
        </Card>
